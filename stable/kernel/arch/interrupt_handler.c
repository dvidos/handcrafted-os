...
void interrupt_handler_c(interrupt_frame_t *frame) {
    ...
    switch (frame->int_no) {
        ...
        case 0x0D:
            ...
            int table = BIT_RANGE(frame->err_code, 2, 1);
            int entry = BIT_RANGE(frame->err_code, 15, 3);
            ...
            log_error_fmt(interrupt_frame_log_formatter, "  ", frame);
            ...
            break;
        case 0x6:
            log_error("Received interrupt %d: Invalid Opcode Exception: The CPU tried to execute an instruction that is not valid", frame->int_no);
            log_error("Trap frame follows:");
            log_error_fmt(interrupt_frame_log_formatter, "  ", frame);
            ...
            break;
        case 0x80:
            extern void isr_syscall(interrupt_frame_t *frame);
            isr_syscall(frame);
            break;
        default:
            log_error("Received interrupt %d (0x%x), error %d", frame->int_no, frame->int_no, frame->err_code);
            log_error("Trap frame follows:");
            log_error_fmt(interrupt_frame_log_formatter, "  ", frame);
            if (++erroneous_interrupts >= 3)
                panic("Too many erroneous interrupts");
