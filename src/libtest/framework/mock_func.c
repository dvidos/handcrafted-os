...
static void mock_func_append_mock_value(mock_func_info_t *info, scalar value);
static void mock_func_append_expected_arg(mock_func_info_t *info, char *arg_name, scalar expected_value);
static scalar mock_func_dequeue_mock_value(mock_func_info_t *info, const char *file, int line);
static scalar mock_func_dequeue_expected_arg(mock_func_info_t *info, char *arg_name, const char *file, int line);
...
static scalar mock_func_dequeue_mock_value(mock_func_info_t *info, const char *file, int line) {
    ...
    scalar value = (scalar)first->payload;
...
static void mock_func_log_debug_info(mock_func_info_t *info) {
    if (!info->mock_values_list->ops->empty(info->mock_values_list)) {
        ...
        for_list(node, info->mock_values_list) {
            testing_framework_setup.printf_value("      - ", node->payload);
        }
    }
    ...
    if (!info->expected_args_list->ops->empty(info->expected_args_list)) {
        ...
        for_list(arg_node,  info->expected_args_list) {
            ...
            for_list(val_node, values_list) {
                testing_framework_setup.printf_value("        - ", val_node->payload);
            }
        }
    }
}
