...
// and https://wiki.osdev.org/Random_Number_Generator
...
static uint32_t z = 362436069;
static uint32_t w = 521288629;
...
void srand(uint64_t seed) {
    // feed the seed semi-uniformly to our two numbers
    z = (((seed >> 32) & 0xFFFF) << 16) | (((seed >> 16) & 0xFFFF) << 0);
    w = (((seed >>  0) & 0xFFFF) << 16) | (((seed >> 48) & 0xFFFF) << 0);
}
...
void srand_time() {
    ...
    // the whole clock makes for only 24 bits
    // 12 months a year -> 4 bits
    // 31 days in month -> 6 bits
    //  7 days in week -> 3 bits
    // 24 hours -> 5 bits
    // 60 mins or secs -> 6 bits
    // 1 hour uptime will occupy 22 bites
    // 5 mins uptime will occupy 19 bits
    // we only get 48 bits of randomness....

    uint64_t seed = 
        ((uint64_t)ct.years << 50) |
        ((uint64_t)ct.months << 46) |
        ((uint64_t)ct.days << 40) |
        ((uint64_t)ct.dow << 37) |
        ((uint64_t)ct.hours << 32) |
        ((uint64_t)ct.minutes << 26) |
        ((uint64_t)ct.seconds << 20) |
        up; 
