...
static int range_block_get_last_block_no(mounted_data *mt, uint32_t range_block_no, uint32_t *last_block_no) {
    ...
    err = v2_range_array_last_block_no((block_range *)mt->generic_block_buffer, ranges_per_block, last_block_no);
    if (err != OK) return err;
...
static int range_block_initialize(mounted_data *mt, uint32_t range_block_no, uint32_t *new_block_no) { // assumes range_block is already allocated
    ...
    int ranges_per_block = mt->superblock->block_size_in_bytes / sizeof(block_range);
    err = v2_range_array_expand((block_range *)mt->generic_block_buffer, ranges_per_block, mt->bitmap, new_block_no, &overflown);
    if (err != OK) return err;
...
static int range_block_release_indirect_block(mounted_data *mt, uint32_t indirect_block_no) {
    ...
    int ranges_per_block = mt->superblock->block_size_in_bytes / sizeof(block_range);
    v2_range_array_release_blocks((block_range *)mt->generic_block_buffer, ranges_per_block, mt->bitmap);
    return OK;
}
