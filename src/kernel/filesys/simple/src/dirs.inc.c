...
static int dir_entry_find(mounted_data *mt, cached_inode *dir_inode, const char *name, uint32_t *inode_id, uint32_t *entry_rec_no) {
    saved_dir_entry entry;

    for (int rec_no = 0; rec_no < DIR_ENTRY_MAX_REC_NO; rec_no++) {
        int bytes = inode_read_file_bytes(mt, dir_inode, rec_no * sizeof(saved_dir_entry), &entry, sizeof(saved_dir_entry));
...
static int dir_entry_update(mounted_data *mt, cached_inode *dir_inode, int entry_rec_no, const char *name, uint32_t inode_id) {
    ...
    int bytes = inode_write_file_bytes(mt, dir_inode, entry_rec_no * sizeof(saved_dir_entry), &entry, sizeof(saved_dir_entry));
    if (bytes < 0) return bytes;
...
static int dir_entry_append(mounted_data *mt, cached_inode *dir_inode, const char *name, uint32_t inode_id) { 
    ...
    int bytes = inode_write_file_bytes(mt, dir_inode, dir_inode->inode_in_mem.file_size, &entry, sizeof(saved_dir_entry));
    if (bytes < 0) return bytes;
...
static int dir_entry_delete(mounted_data *mt, cached_inode *dir_inode, int entry_rec_no) {
    int recs_in_file = dir_inode->inode_in_mem.file_size / sizeof(saved_dir_entry);
    if (entry_rec_no == recs_in_file - 1) {
        // shorten the file, but keep blocks for simplicity
        dir_inode->inode_in_mem.file_size = (recs_in_file - 1) * sizeof(saved_dir_entry);
        return OK;
    } else {
...
static void dir_dump_debug_info(mounted_data *mt, cached_inode *dir_inode, int depth) {
    struct saved_dir_entry entry;
    int rec_no;
    int err, bytes;

    for (rec_no = 0; 0x7FFFFFFF; rec_no++) {
        bytes = inode_read_file_bytes(mt, dir_inode, rec_no * sizeof(saved_dir_entry), &entry, sizeof(saved_dir_entry));
        if (bytes < 0) break;
        ...
        if (is_special) {
            ...
            cached_inode *child_node; // TODO: find a way to not mess with cache for this
            err = get_cached_inode(mt, entry.inode_id, &child_node);
            if (err != OK) break;

            inode_dump_debug_info("", &child_node->inode_in_mem);
            if (child_node->inode_in_mem.is_dir)
                dir_dump_debug_info(mt, child_node, depth + 1);
        }
    }
    if (rec_no == 0) {
        printf("%*s(no entries)\n", depth * 4, "");
