...
struct cache_layer_data {
    ...
    uint8_t *used_slots_bitmap; // array of sufficient bits
...
static int read_slot_from_disk(cache_layer_data *d, int block_no, cache_slot *slot) {
    uint32_t first_sector = block_no * d->block_size_in_sectors;
    void *target = slot->data_ptr;
    for (int i = 0; i < d->block_size_in_sectors; i++) {
        int err = d->device->read_sector(d->device, first_sector + i, target);
        if (err != OK) return err;
        target += d->sector_size_in_bytes;
    }
...
static int write_slot_to_disk(cache_layer_data *d, cache_slot *slot) {
    uint32_t first_sector = slot->block_no * d->block_size_in_sectors;
    void *source = slot->data_ptr;
    for (int i = 0; i < d->block_size_in_sectors; i++) {
        int err = d->device->write_sector(d->device, first_sector + i, source);
        if (err != OK) return err;
        source += d->sector_size_in_bytes;
    }
    slot->is_dirty = 0;
    return OK;
}
...
static int ensure_block_in_cache_slot(cache_layer_data *d, uint32_t block_no, cache_slot **slot_ptr) {
    int err;
...
static int cache_read(cache_layer *cl, uint32_t block_no, int offset_in_block, void *buffer, int length) {
    ...
    if (offset_in_block + length > data->block_size_in_bytes)
        return ERR_OUT_OF_BOUNDS;

    cache_slot *slot;
    int err = ensure_block_in_cache_slot(data, block_no, &slot);
    if (err != OK) return err;
...
static int cache_wipe(cache_layer *cl, uint32_t block_no) {
    cache_layer_data *data = (cache_layer_data *)cl->data;
    ...
    // now that we have it, write it, flag as dirty
    memset(slot->data_ptr, 0, data->block_size_in_bytes);
    slot->is_dirty = 1;
    return OK;
}
...
static int cache_free_memory(cache_layer *cl) {
    ...
    mem->release(mem, data->used_slots_bitmap);
...
cache_layer *new_cache_layer(mem_allocator *memory, sector_device *device, int block_size_bytes, int blocks_to_cache) {
    ...
    data->block_size_in_bytes = block_size_bytes;
    data->block_size_in_sectors = block_size_bytes / device->get_sector_size(device);
    data->sector_size_in_bytes = device->get_sector_size(device);

    data->used_slots_bitmap_size = (blocks_to_cache + 7) / 8;
    data->used_slots_bitmap = memory->allocate(memory, data->used_slots_bitmap_size);
    ...
    memset(data->used_slots_bitmap, 0, data->used_slots_bitmap_size);
