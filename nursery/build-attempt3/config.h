#pragma once
// auto generated, do not edit, run configure.sh to generate
#define VERSION                   "0.1.0"
#define GIT_HASH                  "06f3527-dirty"
#define DATE_BUILT                "2026-04-08 21:30"
#define TRACE_RETURNS             1
#define STAGE1_LOAD_ADDRESS       0x7C00
#define STAGE1_STACK_TOP          0x0000
#define STAGE1_SIZE               512
#define STAGE1_FIRST_SECTOR       0
#define STAGE1_SECTOR_COUNT       1
#define STAGE2_LOAD_ADDRESS       0x0800
#define STAGE2_STACK_TOP          0x7C00
#define STAGE2_SIZE_KB            12
#define STAGE2_FIRST_SECTOR       1
#define STAGE2_SECTOR_COUNT       24
#define KERNEL_LOAD_ADDRESS       0x8000
#define KERNEL_STACK_TOP          0x9FBFF
#define KERNEL_SIZE_KB            192
#define KERNEL_FIRST_SECTOR       25
#define KERNEL_SECTOR_COUNT       384
#define KERNEL_UTIL_PAGES_ADDRESS  
#define KERNEL_UTIL_PAGES_SIZE_KB  
#define KERNEL_RAMDISK_ADDRESS    
#define KERNEL_RAMDISK_SIZE_KB    
#define KERNEL_HEAP_ADDRESS       
#define KERNEL_HEAP_SIZE_KB       
#define KERNEL_PMM_BITMAP_ADDR    
#define KERNEL_PMM_BITMAP_SIZE_KB 
#define KERNEL_CUTOFF_ADDRESS     
#define SECTOR_SIZE               512
#define DISK_IMAGE_SIZE_MB        8
#define DISK_IMAGE_SIZE_BYTES     8388608
#define DISK_IMAGE_SECTOR_COUNT   16384
#define PARTITION_1_FIRST_SECTOR  409
#define PARTITION_1_SECTOR_COUNT  15975
