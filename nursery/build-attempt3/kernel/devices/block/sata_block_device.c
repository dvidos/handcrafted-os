...
typedef struct sata_private_data {
    int port_no;
    HBA_PORT *port;
} sata_private_data;
...
static error_t sata_rw_operation(bool read, HBA_PORT *port, uint32_t sector_lo, uint32_t sector_hi, uint32_t count, uint8_t *buffer) {
...
static error_t block_dev_read(block_device_t *dev, uint64_t lba, uint32_t count, void *buffer) {
    sata_private_data *priv_data = (sata_private_data *)dev->priv_data;
    return sata_rw_operation(true, priv_data->port, LOW_DWORD(lba), HIGH_DWORD(lba), count, buffer);
}

static error_t block_dev_write(block_device_t *dev, uint64_t lba, uint32_t count, const void *buffer) {
    sata_private_data *priv_data = (sata_private_data *)dev->priv_data;
    return sata_rw_operation(false, priv_data->port, LOW_DWORD(lba), HIGH_DWORD(lba), count, (void *)buffer);
}
...
static error_t discover_and_register_serial_ata_storage_device(pci_device_t *pci_dev) {
    ...
    for (int port_no = 0; port_no < 32; port_no++) {
        ...
        char id_buffer[16];
        char name_buffer[64];
        sprintfn(id_buffer, sizeof(id_buffer), "sata%d", port_no);
        sprintfn(name_buffer, sizeof(name_buffer), "SATA hard disk, port #%d", port_no);
        
        sata_private_data *priv_data = kmalloc(sizeof(sata_private_data));
        memset(priv_data, 0, sizeof(sata_private_data));
        priv_data->port_no = port_no;
        priv_data->port = port;

        block_device_t *block_dev = create_block_device(
            id_buffer,
            name_buffer,
            999999,  // total blocks, how to discover???
            512,
            &sata_ops,
            priv_data
        );
...
void discover_and_register_all_sata_block_devices(list_t *pci_devices_list) {
    list_foreach(pci_devices_list, pci_device_t, dev) {
        if (dev->config.class_type != 0x01 || dev->config.sub_class != 0x06) // SATA controllers
            continue;
        
        discover_and_register_serial_ata_storage_device(dev);
    }
}
