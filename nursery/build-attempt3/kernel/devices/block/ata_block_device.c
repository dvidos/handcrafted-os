...
typedef struct ata_private_data {
    ...
    char drive_no; // 0-3, an index into the drives array
} ata_private_data_t;
...
static void write_register(struct ide_channel *channel, uint8_t reg_no, uint8_t value) {
    ...
    if      (reg_no < 0x08) outb(channel->base_port  + reg_no - 0x00, value);
    else if (reg_no < 0x0C) outb(channel->base_port  + reg_no - 0x06, value);
    else if (reg_no < 0x0E) outb(channel->ctrl_port  + reg_no - 0x0A, value);
    else if (reg_no < 0x16) outb(channel->bus_master_ide + reg_no - 0x0E, value);
...
static uint8_t read_register(struct ide_channel *channel, uint8_t reg_no) {
    ...
    if      (reg_no < 0x08) result = inb(channel->base_port + reg_no - 0x00);
    else if (reg_no < 0x0C) result = inb(channel->base_port  + reg_no - 0x06);
    else if (reg_no < 0x0E) result = inb(channel->ctrl_port  + reg_no - 0x0A);
    else if (reg_no < 0x16) result = inb(channel->bus_master_ide + reg_no - 0x0E);
...
static void read_buffer_pio(struct ide_channel *channel, uint8_t reg_no, uint32_t *buffer, uint32_t dwords) {
    ...
    while (dwords-- > 0) {
        if      (reg_no < 0x08) result = inl(channel->base_port + reg_no - 0x00);
        else if (reg_no < 0x0C) result = inl(channel->base_port  + reg_no - 0x06);
        else if (reg_no < 0x0E) result = inl(channel->ctrl_port  + reg_no - 0x0A);
        else if (reg_no < 0x16) result = inl(channel->bus_master_ide + reg_no - 0x0E);
...
static error_t poll(struct ide_channel *channel, bool advanced_check) {
...
/* drive is the drive number which can be from 0 to 3.
   ...
   numsects is the number of sectors to be read, it is a char, 
   as reading more than 256 sector immediately may performance issues. 
   If numsects is 0, the ATA controller will know that we want 256 sectors.
...
*/
static error_t ata_rw_operation(driver_data_t *data, uint8_t direction, uint8_t drive_no, uint32_t lba, uint8_t numsects, uint16_t data_seg, uint32_t data_ptr) {
...
static error_t read_sectors(driver_data_t *data, uint8_t drive_no, uint32_t lba, uint8_t numsects, uint16_t es, uint32_t edi)
{
...
static error_t write_sectors(driver_data_t *data, uint8_t drive_no, uint32_t lba, uint8_t numsects, uint16_t es, uint32_t edi) {
...
static error_t block_device_read(block_device_t *dev, uint64_t lba, uint32_t count, void *buffer) {
    ata_private_data_t *pd = (ata_private_data_t *)dev->priv_data;
    return read_sectors(pd->driver_data, pd->drive_no, lba, count, 0, (uint32_t)buffer);
}

static error_t block_device_write(block_device_t *dev, uint64_t lba, uint32_t count, const void *buffer) {
    ata_private_data_t *pd = (ata_private_data_t *)dev->priv_data;
    return write_sectors(pd->driver_data, pd->drive_no, lba, count, 0, (uint32_t)buffer);
}

static error_t block_device_flush(block_device_t *dev) {
    ata_private_data_t *pd = (ata_private_data_t *)dev->priv_data;
    struct ide_drive *drive = &pd->driver_data->drives[(int)pd->drive_no];
    struct ide_channel *channel = &pd->driver_data->channels[drive->channel_no];
    log_trace("IDE: block_device_flush(drive_no=%d)", pd->drive_no);
    
    uint8_t flush_cmd = (drive->command_sets & (1 << 26)) ? ATA_CMD_CACHE_FLUSH_EXT : ATA_CMD_CACHE_FLUSH;
    write_register(channel, ATA_REG_COMMAND, flush_cmd);
    poll(channel, false);
    return OK;    
}

static void block_device_destroy(block_device_t *dev) {
    // depends on what the private data is
...
static void discover_and_register_mass_storage_pci_device(pci_device_t *pci_dev) {
    ...
    driver_data_t *driver_data = kmalloc(sizeof(driver_data_t));
    memset(driver_data, 0, sizeof(driver_data));

    uint32_t bar0 = pci_dev->config.headers.h00.bar0 & 0xFFFFFFFC;
    uint32_t bar1 = pci_dev->config.headers.h00.bar1 & 0xFFFFFFFC;
    uint32_t bar2 = pci_dev->config.headers.h00.bar2 & 0xFFFFFFFC;
    uint32_t bar3 = pci_dev->config.headers.h00.bar3 & 0xFFFFFFFC;
    uint32_t bar4 = pci_dev->config.headers.h00.bar4 & 0xFFFFFFFC;
    ...
    for (int channel_no = 0; channel_no < 2; channel_no++) {
        for (int master_slave = 0; master_slave < 2; master_slave++) {
            ...
            if (!probe_drive(drive, channel, buffer))
                continue;
            log_debug("Posive probe on ATA drive_no %d", drive_no);
            
            char id[16];
            char name[128];
            sprintfn(id, sizeof(id), "hd%d", drive_no);
            sprintfn(name, sizeof(name), "ATA %s %s drive (%s)", 
                drive->channel_no == 0 ? "primary" : "secondary",
                drive->master_slave == 0 ? "master" : "slave",
                drive->model);

            ata_private_data_t *priv_data = (ata_private_data_t *)kmalloc(sizeof(ata_private_data_t));
            priv_data->pci_dev = pci_dev;
            priv_data->driver_data = driver_data;
            priv_data->drive_no = drive_no;
            block_device_t *dev = create_block_device(
                id, name, 
                drive->size,
                512,
                &block_device_ops,
...
void discover_and_register_all_ata_block_devices(list_t *pci_devices_list) {
    list_foreach(pci_devices_list, pci_device_t, dev) {
        if (dev->config.class_type != 0x01 || dev->config.sub_class != 0x01) // IDE controllers
            continue;
        
        discover_and_register_mass_storage_pci_device(dev);
    }
}
