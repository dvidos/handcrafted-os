...
#include "drivers.h"
#include "partition_deprecated.h"
...
typedef struct superblock {
    ...
    struct partition_deprecated *partition;
    struct file_ops *ops;
    lock_t write_lock;
...
struct file_ops {
    ...
    // get root inode, not freed, can point to static data
    int (*root_dir_inode)(struct superblock *sb, inode_t **n);

    // find a name in a directory, and populate an inode
    ...
    // unlink a file
    int (*unlink)(inode_t *parent_dir, char *name);

    // make a directory (maybe the . and .. as well?)
    int (*mkdir)(inode_t *parent_dir, char *name);

    // remove a directory (maybe the . and .. as well?)
    int (*rmdir)(inode_t *parent_dir, char *name);
};
