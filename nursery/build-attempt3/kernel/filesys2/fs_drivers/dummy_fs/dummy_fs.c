...
static int _dummy_fs_probe(block_device_t *dev) {
    // just check if device seems to contain a supported filesystem
    return OK;
}

static int _dummy_fs_mount(superblock_t *sb) {
    // create private data, store on sb->driver_priv_data
    return OK;
}

static int _dummy_fs_unmount(superblock_t *sb) {
    // destroy private data from sb->driver_priv_data
    return OK;
}

static int _dummy_fs_sync(superblock_t *sb) {
    return OK;
}
...
static int _dummy_fs_lookup(file_descriptor_t *dir, const char *name, file_descriptor_t **out) {
...
static int _dummy_fs_open(file_descriptor_t *fd, int flags, open_file_t **file_handle) {
...
static int _dummy_fs_close(open_file_t *file) {
    // destroy private data from file->driver_priv_data
    return OK;
}

static int _dummy_fs_read(open_file_t *file, void *buf, size_t len, off_t offset) {
...
static int _dummy_fs_write(open_file_t *file, const void *buf, size_t len, off_t offset) {
    // grab private data and offset from file
    return ERR_NOT_PERMITTED;
}

static int _dummy_fs_flush(open_file_t *file) {
    return OK;
}

static int _dummy_fs_opendir(file_descriptor_t *dir, open_file_t **dir_handle) {
...
static int _dummy_fs_readdir(open_file_t *dir_handle, struct dirent *out) {
...
static int _dummy_fs_rewinddir(open_file_t *dir_handle) {
    dir_handle->offset = 0;
    return OK;
}

static int _dummy_fs_closedir(open_file_t *dir_handle) {
    // destroy private data from dir_handle->driver_priv_data
    return OK;
}

static int _dummy_fs_mkdir(file_descriptor_t *parent, const char *name) { 
    // create directory, but also "." and ".."
    return ERR_NOT_PERMITTED;
}

static int _dummy_fs_rmdir(file_descriptor_t *parent, const char *name) {
    // check if dir is empty or not.
    // remove "." and ".."
    return ERR_NOT_PERMITTED;
}

static int _dummy_fs_create(file_descriptor_t *parent, const char *name, int type, file_descriptor_t **out) {
    return ERR_NOT_PERMITTED;
}

static int _dummy_fs_unlink(file_descriptor_t *parent, const char *name) {
    // remove directory entry.
    // if inode counter reaches zero, remove file and blocks as well.
    return ERR_NOT_PERMITTED;
}

static int _dummy_fs_stat(file_descriptor_t *fd, struct stat *out) {
    return ERR_NOT_IMPLEMENTED;
}

static int _dummy_fs_truncate(file_descriptor_t *fd, size_t size) {
    return ERR_NOT_IMPLEMENTED;
}
