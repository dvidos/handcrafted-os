...
error_t elf_get_loading_information(open_file_t *file, virt_addr_t *virt_addr_start, virt_addr_t *virt_addr_end, virt_addr_t *entry_point) {
    ...
    *entry_point = elf_header->entry;
    ...
    err = vfs_seek(file, elf_header->phoff, SEEK_SET);
    if (err < 0) goto exit;
    ...
    *virt_addr_start = lowest_virtual_address;
    *virt_addr_end = highest_virtual_address;
    err = OK;
...
error_t elf_load_into_memory(open_file_t *file) {
    ...
    for (int i = 0; i < elf_header->phnum; i++) {
        ...
        err = vfs_seek(file, program->p_offset, SEEK_SET);
        if (err < 0) goto exit;
