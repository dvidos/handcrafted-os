...
* then...
  ...
  * text/graphics mode still an option on stage2.
  * make boot into graphics, mounting the root filesystem part of the image
  * make a VT100 emulator for a tty window
  ...
  handcrafted-os
    ...
    stable
      ...
      rootfs      (root filesystem contents)
        etc
        bin       (init, sh, edit, etc)
        usr       (lib, include, src)
        var
        tmp
      tools       (tools and scripts for building)
        sfs
        make-rootfs.sh
        make-
      build-tmp   (generated by build)
        stage1
        stage2
        kernel
        libc      (libc.a + headers, to allow for apps)
        apps
        rootfs
      image
        os.img    (final deliverable)
    makefile
    config.mk     <-- version nums, memory addresses, configs etc.
